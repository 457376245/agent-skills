---
name: "omp-requirement-doc-tracking"
description: "OMP-optimized Chinese requirement implementation tracking for feature delivery and review remediation. Use in Oh My Pi for new user-facing requirements, explicit documentation trails, or final alignment after an OMP Requirement Doc Review. Generic bugs, investigations, reviews, and simple edits do not trigger it."
---

# OMP Requirement Doc Tracking
...
- Code review, audit, verification, or acceptance review; use `omp-requirement-doc-review` only when applicable.
...
- Keep implementation status, review status, and the latest review conclusion separate. Do not write a formal review conclusion; that belongs to `omp-requirement-doc-review`.
...
A stateless `completion(model: "smol")` used only to propose a patch does not count as delegated implementation when the main agent reads the affected implementation and call sites, reviews and corrects the proposal, applies every workspace edit itself, and owns all validation and documentation alignment. A tool-using subagent that edits files or owns validation remains delegated implementation.
...
- The review entry identifies whether an independent `omp-requirement-doc-review` pass is recommended and where its report should go.
...
Do not perform the independent delivery review here. When the user asks to review, audit, verify, accept,复核,验收, or check whether an implemented requirement meets its tracking document, use `omp-requirement-doc-review` instead.
